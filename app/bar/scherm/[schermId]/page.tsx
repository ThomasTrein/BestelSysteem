...
export default function SchermPage() {
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const screenDoc = await getDoc(doc(db, 'events', ev.id, 'screens', schermId));
      if (!screenDoc.exists()) { setLoading(false); return; }
      setScreen({ id: screenDoc.id, ...screenDoc.data() } as BarScreen);
  ...
  // Build set of matching category names from categoryIds
  ...
  function itemMatches(item: OrderItem): boolean {
    return screenCategoryNames.has(item.categoryName) || screenItemIds.has(item.itemId);
  }

  function orderMatches(order: Order): boolean {
    return order.items.some(itemMatches);
  }

  const filteredOrders = orders.filter(orderMatches);
  const besteld = filteredOrders.filter((o) => o.status === 'besteld');
  const klaar = [...filteredOrders.filter((o) => o.status === 'klaar')].reverse();
      ...
      <header className="bg-gray-800 border-b border-gray-700 px-4 py-3 shadow-md">
        <div className="flex flex-wrap justify-between items-center gap-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <a href="/bar" className="text-gray-400 hover:text-white text-sm transition-colors">
              ← Terug naar bar
            </a>
        ...
        <main className="px-4 py-4">
          ...
          {besteld.length > 0 && (
            <div className={`grid ${colClass[columns] || 'grid-cols-2'} gap-4 mb-6`}>
              {besteld.map((o) => (
                <SchermOrderCard key={o.id} order={o} fmt={fmt} onToggle={() => toggleStatus(o)} itemMatches={itemMatches} />
              ))}
            </div>
          )}
          {showKlaar && (
...
function SchermOrderCard({
  ...
  onToggle: () => void;
  itemMatches: (item: OrderItem) => boolean;
}) {
          ...
          <div key={gi}>
            ...
            <div className="space-y-1">
              {group.items.map((item, i) => {
                ...
                return (
                  <div key={i} className={`rounded-lg px-3 py-2 border ${matches ? 'bg-green-500/10 border-green-500/30' : 'bg-gray-700/50 border-gray-700/50 opacity-50'}`}>
                    <div className="flex items-baseline justify-between gap-2">
                      <p className={`text-base leading-tight ${matches ? 'text-gray-100' : 'text-gray-400'}`}>
                        <span className={`font-bold text-lg ${matches ? 'text-white' : 'text-gray-500'}`}>{item.quantity}×</span> {item.name}
                      </p>
